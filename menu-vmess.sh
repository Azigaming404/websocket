...
BURIQ () {
    curl -sS https://cybervpn.serv00.net/Autoscript-by-azi-main/izin > /root/tmp
...
Name=$(curl -sS https://cybervpn.serv00.net/Autoscript-by-azi-main/izin | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://cybervpn.serv00.net/Autoscript-by-azi-main/izin | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
...
timestamp=$(sqlite3 /etc/xray/xray.db "SELECT exp FROM vmess WHERE username = '$username';")


exp=$(date -d "@$timestamp" +"%Y-%m-%d")
...
function renewvmess(){
  ...
  echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}"
  echo -e "$COLOR1┌─────────────────────────────────────────────────┐${NC}"
  NUMBER_OF_CLIENTS=$(sqlite3 /etc/xray/xray.db "SELECT COUNT(*) FROM vmess;")
  if [[ ${NUMBER_OF_CLIENTS} == '0' ]]; then
    ...
    sqlite3 /etc/xray/xray.db "SELECT username, exp FROM vmess;" | sort | uniq | nl

    echo -e "$COLOR1│${NC}"
    echo -e "$COLOR1│${NC}  • [NOTE] Press any key to back on menu"
    echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}" 
    echo -e "$COLOR1┌────────────────────── BY ───────────────────────┐${NC}"
    echo -e "$COLOR1│${NC}                 • SwiftGuard Lite •                 $COLOR1│$NC"
    echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}" 
    echo -e "$COLOR1───────────────────────────────────────────────────${NC}"
    read -rp "Input Username : " user
    if [ -z "$user" ]; then
      ...
      timestamp=$(sqlite3 /etc/xray/xray.db "SELECT exp FROM vmess WHERE username = '$user';")
      exp=$(date -d "@$timestamp" +"%Y-%m-%d")
      ...
      sqlite3 /etc/xray/xray.db "UPDATE vmess SET exp = strftime('%s', '$exp') WHERE username = '$user';"

      sed -i "/### $user/c\### $user $exp" /etc/vmess/.vmess.db
      systemctl restart xray > /dev/null 2>&1
      clear
      echo -e "$COLOR1┌─────────────────────────────────────────────────┐${NC}"
      echo -e "$COLOR1│${NC} ${COLBG1}             • RENEW VMESS USER •              ${NC} $COLOR1│$NC"
...
user_exists=$(sqlite3 /etc/xray/xray.db "SELECT COUNT(*) FROM vmess WHERE username = '$user';")
...
if [[ ${user_exists} -gt '1' ]]; then
  ...
  echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}"
  echo -e "$COLOR1┌─────────────────────────────────────────────────┐${NC}"
  echo -e "$COLOR1│${NC} User $user sudah ada di database.                │"
  echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}" 
  echo -e "$COLOR1┌────────────────────── BY ───────────────────────┐${NC}"
  echo -e "$COLOR1│${NC}            • SwiftGuard Lite •                   $COLOR1│$NC"
  echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}" 
...
sqlite3 /etc/xray/xray.db "INSERT INTO vmess (username, uuid, exp) VALUES ('$user', '$uuid', '$exp');"
